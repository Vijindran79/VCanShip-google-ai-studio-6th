...
const mockSchedules: Schedule[] = [
  ...
  { id: 's3', mode: 'AIR', carrier: 'MSC Air Cargo', origin: { code: 'ORD', name: 'Chicago O\'Hare'}, destination: { code: 'CAN', name: 'Guangzhou Baiyun' }, route: 'ORD → CAN', etd: '2024-07-26', eta: '2024-07-28', transit_days: 2, reliability_pct: 92, flight: '5Y340', capacity_left: '2 t' },
  ...
  { id: 's8', mode: 'AIR', carrier: 'Cathay Cargo', origin: { code: 'HKG', name: 'Hong Kong'}, destination: { code: 'LHR', name: 'London Heathrow' }, route: 'HKG → LHR', etd: '2024-07-28', eta: '2024-07-29', transit_days: 1, reliability_pct: 93, flight: 'CX251', capacity_left: '5 t' },
...
function renderSchedulesPage() {
    ...
    page.innerHTML = `
        ...
        <div class="schedules-layout">
            <aside class="schedules-filters-panel">
                 <h3>Filters</h3>
            ...
            <main class="schedules-results-panel">
...
/**
 * Renders the final, sorted list of schedules into the table body.
 * @param schedules The array of schedules to render.
 */
function renderTableRows(schedules: Schedule[]) {
    // Sort by ETD, ascending
    schedules.sort((a, b) => new Date(a.etd).getTime() - new Date(b.etd).getTime());

    const tableBody = DOMElements.schedules.tableBody;
    if (schedules.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="10" style="text-align: center; padding: 2rem;">No schedules match your criteria.</td></tr>`;
        return;
    }

    const modeIcons = {
        SEA: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mode-icon"><path stroke-linecap="round" stroke-linejoin="round" d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.82m5.84-2.56a17.96 17.96 0 0 0-5.84-2.56m0 0A17.965 17.965 0 0 1 12 2.25a17.965 17.965 0 0 1 5.841 2.56m-8.481 4.68-4.68 8.482a2.25 2.25 0 0 1-3.182 0l-1.26-1.26a2.25 2.25 0 0 1 0-3.182l8.48-4.681a2.25 2.25 0 0 1 3.182 0l1.26 1.26a2.25 2.25 0 0 1 0 3.182Z" /></svg>`,
        AIR: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mode-icon"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" /></svg>`,
    };

    tableBody.innerHTML = schedules.map(s => `
        <tr class="schedule-row">
            <td class="mode-cell">${modeIcons[s.mode]} ${s.mode}</td>
            <td>${s.carrier}</td>
            <td class="port-cell"><strong>${s.origin.name}</strong><span>${s.origin.code}</span></td>
            <td class="port-cell"><strong>${s.destination.name}</strong><span>${s.destination.code}</span></td>
            <td>${s.vessel || s.flight || 'N/A'}</td>
            <td>${s.etd}</td>
            <td>${s.eta}</td>
            <td>${s.transit_days} d</td>
            <td class="${s.reliability_pct >= 90 ? 'reliability-high' : 'reliability-low'}">${s.reliability_pct}%</td>
            <td>${s.capacity_left || 'N/A'}</td>
        </tr>
    `).join('');
}
...
function applyFiltersAndRender() {
    ...
    renderTableRows(finalFilteredSchedules);
}
